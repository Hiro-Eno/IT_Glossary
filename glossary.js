...
{target:`DNSSEC（Domain Name System Security Extensions）`, content:`ネームサーバーからの応答を検証する手法。DNSによってホスト名に対応するIPアドレスを取得する時、偽のDNAサーバーから偽装された情報が送られないように、公開鍵暗号や電子署名の仕組みを使って検証する手法。対応しているDNAサーバーが応答時に電子署名を付加し、受信側がその電子署名を検証することで、正規のDNSサーバーからの応答であることを確認できる。`},
...
{target:`k-means法`, content:`最初にkこの適当なクラスタにわけたあと、それぞれのクラスタでの平均（重心）を計算することを繰り返すことで、似た様なデータを集める手法。`},
...
{target:`走る`, content:`プログラムなどのソフトウェアが動くことを指す。`},
