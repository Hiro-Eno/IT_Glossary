...
{target:`エッジコンピューティング`, content:`端末側で処理してサーバーのふかや遅延を減らす。`},
...
{target:`RPA`, content:`事務処理などの業務を自動化する。`},
{target:`LPWA`, content:`低消費電力で長距離の通信を実現した無線通信技術。`},
{target:`LTE-M`, content:`LTEの空いた領域を使用するIoT通信規格。`},
{target:`NGN`, content:`次世代のネットワーク基盤。`},
{target:`IPoE`, content:`LANと同じ方法でインターネットに接続する接続方式。`},
{target:`MaaS`, content:`ITを活用してあらゆる交通機関を統合して利用。`},
{target:`HCI`, content:`1台でシンプルなサーバー構成を実現できるインフラ製品。`},
{target:`マイクロサービス`, content:`複数の小さなサービスに分類してアプリケーションを構成。`},
{target:`ブロックチェーン`, content:`分散型ネットワークと暗号技術を使ったデータ管理手法。`},
{target:`xR（VR、AR、MR）`, content:`現実世界と仮想世界の融合。`},
{target:`LiDAR`, content:`反射を利用した距離の測定。`},
{target:`オムニチャネル`, content:`ネットとリアルを融合したマーケティング手法。`},
{target:`オンプレミス`, content:`サーバーを自社で構築・運用。`},
{target:`VUI`, content:`スマートスピーカーで広がる音声認識のインターフェイス。`},
{target:`サブスクリプション`, content:`利用料を支払って期間内だけ利用する契約。`},
{target:`NFC`, content:`非接触のICチップで実現した近距離の無線通信。`},
{target:`フィンテック`, content:`ITと金融の融合。`},
{target:`SDGs`, content:`世界が抱える諸問題に対するゴール。`},
{target:`デジタルツイン`, content:`サイバー空間上に物理空間を再現する。`},
{target:`eSIM`, content:`交換せずにキャリアや通信プランを変更できるSIM。`},
{target:`GIGAスクール構想`, content:`学校におけるICTの活用。`},
...
{target:`OTG`, content:`スマホとUSB機器を直接接続できる企画。`},
{target:`STEM教育とSTEAM教育`, content:`理数教育に創造性教育を加えた教育概念。`},
{target:`ドッグフーディング`, content:`社内で自社の製品を使ってテストする。`},
{target:`★エンジニア1年目から知っておきたい基本用語`, content:`ーーーーー`},
{target:`GDPR`, content:`EUにおける個人情報保護の枠組み。`},
{target:`CCPA`, content:`カリフォルニア州における個人情報保護の枠組み。`},
{target:`CxO（CTO,CIO,CISO,…）`, content:`企業における様々な責任者の総称と呼称。`},
{target:`VPoE`, content:`エンジニアを率いるマネジメント責任者。`},
{target:`テックリード`, content:`技術面でチームを引っ張り、他チームと連携する。`},
{target:`EM`, content:`エンジニアの技術力や働き方を最大化する。`},
{target:`フルスタック`, content:`1人で幅広い領域の技術を網羅。`},
{target:`1 on 1`, content:`部下の成長をサポートする定期的なミーティング。`},
{target:`OKR`, content:`会社の目標と個人の目標を連携させるフレームワーク`},
...
{target:`インダストリー4.0`, content:`製造業における新たな産業革命。`},
{target:`PoC`, content:`新しいアイデアを試して投資判断に役立てる。`},
{target:`PLM`, content:`製品の企画から廃棄までを管理する手法。`},
{target:`BPMとBPMS`, content:`企業の業務改善を可視化。`},
{target:`BRMS`, content:`実務におけるルールを管理してシステム開発を効率化。`},
{target:`ERP`, content:`企業の基幹システムで全体を最適化。`},
{target:`SCM`, content:`受注から納品までを一元管理。`},
{target:`CRM`, content:`顧客情報を管理して顧客に合った商品やサービスを提供。`},
{target:`SFA`, content:`企業の営業活動に関するデータを一元管理。`},
{target:`チャットボット`, content:`人工知能を使って顧客の問い合わせ対応を自働化。`},
{target:`DSPとSSP`, content:`Webにおける広告配信を最適化。`},
{target:`RFID`, content:`ID情報を埋め込んだタグで商品などを管理。`},
{target:`アプライアンス`, content:`特定の機能に特化した機器。`},
{target:`DMP`, content:`情報を蓄積し、顧客に合わせて広告を配信する。`},
{target:`KPIとKGI`, content:`企業や部署における目標達成の指標。`},
{target:`チャーンレート`, content:`サブスクリプションの解約率。`},
{target:`MRRとARR`, content:`サブスクリプション型のビジネスの成長を調べる指標。`},
{target:`アジリティ`, content:`社会の変化に追随できる機敏性。`},
{target:`EMS`, content:`製造業におけるアウトソーシング。`},
{target:`Markdown`, content:`手軽に文書の構造を指定できる記法。`},
{target:`ミドルウェア`, content:`OSとアプリの橋渡しをするソフトウェア。`},
