...
{target:`ティーチング`, content:`経験や知識が少ない相手に対して、学校での授業のように技術などを解説すること。<br>その背景や理由なども含めて論知的に解説することが求められる。`},
...
{target:`PoC（Proof of Concept）`, content:`概念実証。<br>新しいアイデアを試して投資判断に役立てる。新しい技術やアイデアなどを実際に試してみることを指す。製品やシステムの簡易版を作ってみて、その実現可能性や社会での需要などを位定めるために実施される。`},
...
{target:`マーケットイン`, content:`顧客側がこれが欲しいというものを重視して、売れすものを作るという考え方。<br>企業の計画よりも顧客の視点で製品を開発、販売すること。`},
...
{target:`アドネットワーク`, content:`複数のWebサイトを取りまとめた広告ネットワーク。<br>DSPは複数のアドネットワークに配信できる。`},
{target:`アドエクスチェンジ`, content:`アドナっとワークが持つ広告枠を交換し、自由に売買する仕組み。`},
{target:`RFID（Radio Frequency IDentification）`, content:`ID情報を埋め込んだタグで商品などを管理。<br>電波や電磁波によって非接触で読み書きする仕組み。近くにあるタグをまとめて読み取れるため、効率よくスキャンできる。箱の中やタグが見えなくても使用できる。NFCと比べて通信距離が長い特徴がある。`},
...
{target:`ページビュー（PV）（Page View）`, content:`Webサイトで特定のページが開かれた回数のこと。<br>アクセス数の指標として使われる。同じ人が複数回アクセスした場合も、それだけカウントされる。`},
...
{target:`ネガティブチャーン`, content:`解約による収益の減少よりも、高額な契約などに変更した利用者などの収益がうわまわったこと。<br>単価を上げた場合などに発生し、経営状況としては悪くない。`},
...
{target:`正規化`, content:`データの規則性に注目して整理。<br>データを利用しやすいように成形すること。関係エータベースの場合、データの冗長性を排除し、データの不整合が起きにくくするために実施され、「第1正規化」、「第2正規化」、「第3正規化」、「ボイスコッド正規化」などがある。正規化されたデータベースで、データを更新する時は1箇所だけど更新すればよくなり、多くの場合、全体のデータ量が減る。`},
{target:`正規表現`, content:`複数の文字列を1つのパターンで識別する表現。<br>複数の文字列を1つの文字列で識別する表現方法で、指定した条件に合致するかを調べるために使われる。`},
...
{target:`コールバック`, content:`ある処理を実行する時その処理の途中で別の処理を実行するように指定する手法。<br>用意されている処理の中で、任意の処理を実行できるようになる。`},
...
{target:`パブサブ（Pub/Sub）`, content:`パブリッシュとサブスクライブの略で「出版-購読型」ち訳される構成。<br>非同期にメッセージをやり取りすることで、結合度が低いためスケーラビリティに優れる。`},
...
{target:`スプリント計画`, content:`プロダクトアックログの上位から今回のスプリントでの項目を選び、そのスプリントでの目標、作業の実行計画を立てること。<br>チームとして作業量を調整する。`},
...
{target:`ビルド（ビルドツール）`, content:`ソースコードから実行ファイルを生成。<br>C言語やJavaなどのコンパイラ型の言語で、ソースコードを作成した後に行うコンパイルやリンクといった作業のこと。複数のソースコードのコンパイルが必要な場合、ビルドツールを使うことで、複雑な手順を自働化できる。変更していないソースコードはコンパイル作業を実施しないことで、コンパイル時間を短縮できる。`},
{target:`make`, content:`自動的に実行する処理を記述したMakefileというファイルを作成すること。<br>複雑な手順でもmakeというコマンドだけで実行できるツール。`},
...
{target:`ライセンス`, content:`OSSの利用や改変、再配布など、従う必要がある規約。<br>代表的なものにGPL（GNU General PublicLicense）やBSD License、MIT Licenseなどがある。`},
...
{target:`DevOps`, content:`開発から保守まで一貫して対応。<br>開発者と運用担当者が密に連携することで、柔軟かつスピーディーにシステムを開発することや組織としての文化のこと。顧客のニーズに速やかに対応するだけでなく、エンジニアも幅広いスキルを身に着けることができる。`},
...
{target:`ライブマイグレーション`, content:`あるコンピュータ上で動作している仮想マシンを、実行したまま他のコンピュータに移すこと。<br>ハードウェアの保守や新機種への更改でも処理を辞めずに移行できる。`},
...
{target:`SPA（Single PageApplication）`, content:`単一のWebページでコンテンツを切り替える仕組み。<br>Webブラウザでリンクをクリックした時にページを遷移することなく新しいページの内容を表示する仕組み。1つのWebページでアプリケーションが構成されており、ページの一部だけを更新すれば済むため、サーバーとの通信量を最小限に抑えられる。また、利用者としてもページを書き換えられる時間が減るためストレスが減る。`},
...
{target:`ブラックハットSEO（Search Engine Optimaization）`, content:`悪質な手法で検索結果を変える。<br>検索結果に表示される順位を不正な方法で変えることを指し、質の低いコンテンツを上位表示させるテクニックを意味する。リンクを購入したり自作自演で被リンクを大量に設置したいする方法や、本文中にキーワードを不自然な形で大量に詰め込む方法、検索エンジン向けと利用者向けに別のページを用意する方法名護が使われる。`},
...
{target:`Iorem ipsum`, content:`画面イメージなどを説明する時に、何らかの文章が入ることを示すために使われるダミーテキスト。<br>文面に意味はなく、デザインと見せるために使われる。`},
...
{target:`ヘッドレスCMS`, content:`見た目を自由に作れるWebコンテンツ管理の仕組み。<br>表示する画面を持たないCMSのこと。ブログなどのように、文章や画像を用意するだけで、だれでも簡単にWebサイトを更新・管理できる便利なシステムとしてCMSがある。ヘッドレスCMSでは閲覧画面を用意せず、コンテンツを管理する機能だけを持つ。別のシステムからAPIを投資てそのコンテンツを呼び出して使われる。`},
...
{target:`マテリアルデザイン`, content:`Googleが提唱するデザインの考え方。<br>ボタンなどに奥行きや影、動きなどを表現し、利用者が現実に即して捉えられるようなデザインにすること。奥行きや影を保湧現するには要素の重なりなどを意識する必要がありアニメーションで動きを表現するにはその物体の重さや質感を体感できる必要がある。統一感のあるデザインにより、利用者のストレスをなくすことにつながる。`},
...
{target:`YAML（YAML Ain't Markup Language）`, content:`XMLやJSONのように設定ファイルなどに使われる書式。<br>インデントで改装を表現する。`},
...
{target:`MPLS（Multi-Protocol Label Switching）`, content:`ラベルを使って転送先を制御する方法。<br>宛先のIPアドレスでネットワークの経路を判断する代わりに、パケットにつけられたラベルと呼ばれる目印で経路を判断する手法。データセンターなど複雑なネットワークでも経路を検索する時にラベルのみを見ればよいため、高速に探索できる。`},
...
{target:`遺伝的アルゴリズム`, content:`生物の進化を模倣したアルゴリズム。<br>強いものが生き残り、弱いものが消えていくという生物の進化の過程を模倣したアルゴリズム。盛大が進むごとに少しずつ進化して最適解に近づける。弱いものも一部残すことで多様性を確保したり、時々突然変異を発生させたりするといった工夫がある。`},
