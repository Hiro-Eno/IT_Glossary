...
{target:`SSR（Server Side Rendering）`, content:`JavaScriptのコートをレンダリングする場所。動的なWebサイトで、HTMLを生成する手法。サーバー側でHTMLを生成してページの全体を返し、Webブラウザでそれを表示する。`},
...
{target:`TC39（Technical Committee 39）`, content:`JavaScriptの開発者や専門家のグループで、JavaScriptの使用（ECMSScript）の策定や議論を行っている。主要なブラウザベンダーも参加している。`},
...
{target:`仮想DOM（Document Object Model）`, content:`ブラウザ内の要素を簡単に操作し高速表示する仕組み。WebブラウザがHTMLファイルを解釈し、表示する時に使うDOMをJavaScriptのオブジェクトとして表現したもの。JavaScriptのプログラムでDOMを書き換えた時に、その差分を反映することで、動的にWebページの内容を書き換えるが、DOMOの変更を最小限に抑えることで、高速に表示する仕組み。`},
...
{target:`CIA（機密性、完全性、可用性）（Confidentiality, Integritym Availability）`, content:`セキュリティの3要素。`},
{target:`PCI DSS（Payment Card Industry Data Security Standard）`, content:`クレジットカード情報を扱う時のセキュリティ基準。`},
{target:`SLA（Service Level Agreement）`, content:`サービス事業者が示す品質の保証内容。`},
...
{target:`FIDOとTPM（Fast IDentity Online, Trusted Platform Module）`, content:`パスワードに代わる認証の規格。`},
{target:`トラッキングとITP（Inteligent Tracking Prevention）`, content:`Web上での人の行動を追跡。`},
{target:`ワンタイムパスワード（OTP）（One-Time Password）`, content:`一定の時間で変わるパスワード。`},
{target:`WPS（Wi-Fi Protected Setup）`, content:`無線LANを簡単に設定する仕組みや規格。`},
{target:`Wi-Fi 6`, content:`高速・多数同時接続を実現した次世代Wi-Fi。`},
{target:`インシデント`, content:`重大な事故・事件につながりかねない事案。`},
{target:`CSIRTとSOC（Computer Sequrity Incident Response Team, Security Operation Center）`, content:`セキュリティに対応する組織。`},
{target:`CVSS（Common Vulnerability Scoreing System）`, content:`脆弱性の深刻さの共通基準。`},
{target:`脆弱性診断`, content:`脆弱性の存在を調べる調査。`},
{target:`UTM（Undified Threat Management）`, content:`複数のセキュリティ機能をまとめた機器。`},
{target:`SIEM（Security Information and Event Management）`, content:`セキュリティに関するログを一元管理する仕組み。`},
{target:`SOAR（Security Orchestration, Automation and Response）`, content:`セキュリティの運用を自働化するシステム。`},
{target:`レピュテーション`, content:`過去の評判データを使い、悪質なものを排除する手法。`},
{target:`遠隔操作ウイルスとRAT（Remote Administration Tool/Remote Access Tool）`, content:`パソコンやスマホに感染して外部から操作。`},
{target:`ビジネスメール詐欺（BEC）（Business Email Compromise）`, content:`取引先になりすまし、金銭的な被害を与える手法。`},
{target:`EPP（Endpoint Protection Platform）`, content:`組織内に侵入したマルウェアを検知するソフトウェア。`},
{target:`サンドボックス`, content:`保護された領域でプログラムを試すソフトウェア。`},
{target:`NGAV（Next Generation Anti-Virus）`, content:`AIなどを用いてマルウェアを検知するソフトウェア。`},
{target:`EDR（Endpoint Detection and Response）`, content:`マルウェアの脅威を検知し、対応を支援するツール。`},
{target:`VPN（Virtual Private Network）`, content:`安全なネットワークを仮想的に構築する手法。`},
{target:`VDI（Virtual Desktop Infrastructure）`, content:`リモートで使うデスクトップ環境を実現する仕組み。`},
{target:`WAF（Web Application Firewall）`, content:`典型的な脆弱性を狙った攻撃を阻止する仕組み。`},
{target:`DLP（Data Loss Prevention）`, content:`機密情報の外部への流出を阻止する技術。`},
{target:`SQLインジェクション`, content:`データベースを扱うアプリケーションの脆弱性。`},
{target:`XSS（Cross Site Scripting）`, content:`複数のサイトを横断する攻撃。`},
{target:`CSRF（Cross-Site Request Forgeries）`, content:`他人に成りすました攻撃。`},
{target:`SDNとNFV（Software-Defined Networking, Network Functions Virtualization）`, content:`仮想的なネットワークを実現。`},
{target:`DNSSEC（Domain Name System Security Extensions）`, content:`ネームサーバーからの応答を検証する手法。`},
{target:`MPLS（Multi-Protocol Label Switching）`, content:`ラベルを使って転送先を制御する方法。`},
{target:`BLE（Bluetooth Low Energy）`, content:`Bluetoothを使った低消費電力の通信。`},
{target:`PLC（Power Line Communications）`, content:`コンセントにさしてインターネットに接続。`},
{target:`VoIP（Voice over Internet Protocol）`, content:`インターネット経由での音声通信。`},
{target:`FQDN（Fully Qualified Domain Name）`, content:`URLなどでドメイン名を省略しない表記方法。`},
{target:`SSL/TLS（Secure Sockets Layer, Transport Layer Sequrity）`, content:`Webにおけるデータを暗号化する仕組み。`},
{target:`PKI（Public Key Infrastructure）`, content:`公開鍵暗号を使用したセキュリティ基盤。`},
...
{target:`ディープラーニング（DL）（Deep Learning）`, content:`深い階層を実現したニューラルネットワーク。`},
